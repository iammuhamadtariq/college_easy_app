...
class AdminTab5:
    def __init__(self, Page):
        ...
        self.title_label = Label (self.page, text=" Manage Admin Record ", font=("times new roman",20,"bold"))
        ...
        # Labels
        self.enter_record_label = Label(self.left_frame, text="Enter admin Record",bg="lightgreen" , font=("arial",18,"bold"), fg="Red")
        self.enter_record_label.grid(row=1, column=0, ipadx=5, ipady=5, pady=10, padx=10, sticky=W)

        self.user_id_label = Label(self.left_frame, text="admin ID",bg="lightgreen" , font=("arial",12,"bold"))
        ...
        self.admin_table = ttk.Treeview(table_frame, columns=("user_id","name","password","gender","DOB","email","address","phone_number","scale","salary"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        ...
        self.admin_table.heading("user_id", text="ID")
        ...
        self.admin_table.heading("DOB", text="DOB")
        self.admin_table.heading("email",text="Email")
        self.admin_table.heading("address",text="Address")
        self.admin_table.heading("phone_number",text="Phone No.")
        self.admin_table.heading("scale",text="scale")
        self.admin_table.heading("salary",text="salary")

        self.admin_table.column("user_id", width=50)
        self.admin_table.column("name",width=75)
        self.admin_table.column("password", width=75)
        self.admin_table.column("gender", width=50)
        self.admin_table.column("DOB", width=75)
        self.admin_table.column("email",width=75)
        self.admin_table.column("address",width=75)
        self.admin_table.column("phone_number",width=75)
        self.admin_table.column("scale",width=75)
        self.admin_table.column("salary",width=75)
    ...
    def update_record(self):
        admin_object = Admin(self.user_id_entry.get(), 
        self.name_entry.get(), 
    ...
    def save_record(self):
        ...
        user_object = User(self.user_id_entry.get(),'admin')
        user_object.setter()
    ...
    def search_data(self):
        ...
        cur.execute("SELECT * FROM adminTable WHERE "+str(self.combo_search.get())+" LIKE '%"+str(self.search_by_entry.get())+"%'")
        rows = cur.fetchall()
        if len(rows)!=0:
            self.admin_table.delete(*self.admin_table.get_children())
            for row in rows:
