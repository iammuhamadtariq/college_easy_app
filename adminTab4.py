...
class AdminTab4:
    def __init__(self, Page):
        ...
        self.title_label = Label (self.page, text=" Manage Accountant Record ", font=("times new roman",20,"bold"))
        ...
        # Labels
        self.enter_record_label = Label(self.left_frame, text="Enter accountant Record",bg="lightgreen" , font=("arial",18,"bold"), fg="Red")
        self.enter_record_label.grid(row=1, column=0, ipadx=5, ipady=5, pady=10, padx=10, sticky=W)

        self.user_id_label = Label(self.left_frame, text="accountant ID",bg="lightgreen" , font=("arial",12,"bold"))
        ...
        self.phone_number_label = Label(self.left_frame, text="scale",bg="lightgreen" , font=("arial",12,"bold"))
        ...
        self.scale_entry['values']=('15th','16th','17th','18th','19th')
        ...
        self.accountant_table = ttk.Treeview(table_frame, columns=("user_id","name","password","gender","DOB","email","address","phone_number","scale","salary"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)

        scroll_x.pack(side=BOTTOM, fill=X)
        scroll_y.pack(side=RIGHT, fill=Y)
        scroll_x.config(command=self.accountant_table.xview)
        scroll_y.config(command=self.accountant_table.yview)

        self.accountant_table['show']='headings'

        self.accountant_table.heading("user_id", text="ID")
        self.accountant_table.heading("name",text="Name")
        self.accountant_table.heading("password", text="Password")
        self.accountant_table.heading("gender", text="Gender")
        self.accountant_table.heading("DOB", text="DOB")
        self.accountant_table.heading("email",text="Email")
        self.accountant_table.heading("address",text="Address")
        self.accountant_table.heading("phone_number",text="Phone No.")
        self.accountant_table.heading("scale",text="scale")
        self.accountant_table.heading("salary",text="salary")

        self.accountant_table.column("user_id", width=50)
        self.accountant_table.column("name",width=75)
        self.accountant_table.column("password", width=75)
        self.accountant_table.column("gender", width=50)
        self.accountant_table.column("DOB", width=75)
        self.accountant_table.column("email",width=75)
        self.accountant_table.column("address",width=75)
        self.accountant_table.column("phone_number",width=75)
        self.accountant_table.column("scale",width=75)
        self.accountant_table.column("salary",width=75)


        self.accountant_table.pack(fill=BOTH, expand=1)
        self.accountant_table.bind("<ButtonRelease-1>",self.get_cursor)
        self.fetch_data()

    def update_record(self):
        accountant_object = Accountant(self.user_id_entry.get(), 
        self.name_entry.get(), 
        ...
        accountant_object.update()

        self.clear_entries()
        self.fetch_data()
        del accountant_object
    ...
    def save_record(self):
        ...
        accountant_object.setter()

        user_object = User(self.user_id_entry.get(),'accountant')
        user_object.setter()
    ...
    def fetch_data(self):
        ...
        cur.execute("SELECT * FROM accountantTable")
        rows = cur.fetchall()
        if len(rows)!=0:
            self.accountant_table.delete(*self.accountant_table.get_children())
            for row in rows:
                self.accountant_table.insert('',END,values=row)
        conn.commit()
        conn.close()


    def get_cursor(self, ev):
        cursor_row = self.accountant_table.focus()
        contents = self.accountant_table.item(cursor_row)
        row = contents['values']
    ...
    def search_data(self):
        ...
        cur.execute("SELECT * FROM accountantTable WHERE "+str(self.combo_search.get())+" LIKE '%"+str(self.search_by_entry.get())+"%'")
        rows = cur.fetchall()
        if len(rows)!=0:
            self.accountant_table.delete(*self.accountant_table.get_children())
            for row in rows:
